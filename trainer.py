"""
trainer.py  —  Orchestrator for SUSY-Regime Engine
===================================================

Loads data → computes SUSY regime signals → builds JSON → uploads.
...
from susy_regime import compute_universe_regime
...
def process_window(args: Tuple) -> Dict:
    ...
    try:
        ...
        result = compute_universe_regime(universe_prices, config_dict, window)
...
def run_trainer(hf_token: Optional[str] = None) -> Dict:
    """Run the full SUSY-Regime pipeline."""
        ...
        for ticker in available:
            ...
            if best_win is not None:
                best_window_per_etf[ticker] = {
                    "z_score": best_z,
                    "window": int(best_win),
                    "energy_gap": safe_float(best_data.get("energy_gap", 0)),
                    "regime_shift_signal": safe_float(best_data.get("regime_shift_signal", 0)),
                    "regime_type": best_data.get("regime_type", "unknown"),
    ...
    # ── Save JSON ─────────────────────────────────────────────────────────────
    logger.info("\n💾 Saving JSON results...")
    tab1_path = f"susy_regime_{run_date}.json"
    tab2_path = f"susy_regime_breakdown_{run_date}.json"
