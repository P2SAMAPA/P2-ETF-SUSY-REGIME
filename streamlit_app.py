...
st.set_page_config(page_title="SUSY-Regime Engine", layout="wide")
...
.badge-hold{background:#f39c12;border-radius:6px;padding:2px 12px;font-size:0.75rem;
            font-weight:700;color:white}
.badge-regime{background:#8e44ad;border-radius:6px;padding:2px 8px;font-size:0.65rem;
              font-weight:700;color:white}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">⚛️ SUSY-Regime Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Supersymmetric Quantum Mechanics · Fokker-Planck Regime Detection · '
    'Energy Gap Early Warning · Regime Shift Prediction</div>',
    unsafe_allow_html=True)
...
def regime_badge(regime_type: str) -> str:
    if "IMMINENT" in regime_type:
        return f'<span class="badge-regime">⚠️ {regime_type}</span>'
    elif "POSSIBLE" in regime_type:
        return f'<span class="badge-regime">🔶 {regime_type}</span>'
    else:
        return f'<span class="badge-regime">✅ {regime_type}</span>'
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.markdown("## ⚛️ SUSY-Regime")
st.sidebar.markdown(f"**Next Trading Day**")
st.sidebar.markdown(f"`{next_trading_day()}`")
st.sidebar.markdown(f"**SUSY Potential:** {config.SUSY['potential_type']}")
st.sidebar.markdown(f"**Superpotential:** {config.SUSY['superpotential']}")
st.sidebar.markdown(f"**Energy Gap Threshold:** {config.SUSY['energy_gap_threshold']}")
st.sidebar.markdown("---")
...
tab1_path = find_latest(files, "susy_regime_")
tab2_path = find_latest(files, "susy_regime_breakdown_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Best Window per ETF — SUSY Energy Gap Signal")

    with st.expander("📖 How SUSY-Regime Works", expanded=True):
        st.markdown("""
**SUSY-Enhanced Regime Detection** uses Supersymmetric Quantum Mechanics:

| Step | What happens |
|------|-------------|
| 1. Fokker-Planck | Model time-series with regime switches |
| 2. SUSY partners | Construct partner Hamiltonians H+ and H- |
| 3. Energy spectrum | Compute eigenvalues of both Hamiltonians |
| 4. Energy gap | Calculate gap between ground and first excited state |
| 5. Early warning | Decreasing gap = impending regime shift |

**Interpretation:**
- **Higher z-score** → Strong stable regime (no shift expected)
- **Lower z-score** → Regime shift imminent (energy gap closing)
    ...
    for universe_name in UNIVERSE_ORDER:
        ...
        if buy_etfs:
            ...
            for idx, (ticker, z_score, data) in enumerate(buy_etfs[:3]):
                ...
                energy_gap = safe_float(data.get("energy_gap", 0))
...
<div class="hero-card">
  ...
  <div class="score">{regime_badge(regime_type)}</div>
  <div class="score">Energy Gap = {energy_gap:.3f}</div>
...
""", unsafe_allow_html=True)
            ...
            if full_scores:
                ...
                for t, info in full_scores.items():
                    ...
                    rows.append({
                        "ETF": t,
                        "z-score": round(z, 4),
                        "Energy Gap": round(safe_float(info.get("energy_gap", 0)), 4),
                        "Regime Type": info.get("regime_type", "unknown"),
    ...
    st.caption(f"Run date: {data1.get('run_date','?')} · Higher z-score = stable regime · Lower z-score = shift imminent")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore SUSY Regime Signals by Window")
...
- **252d** — Annual regime signal: recommended primary
- **504d–1008d** — Medium-term regime structure
- **2016d+** — Very long-run regime detection
- **4032d / 4536d** — Full history regime analysis (2008–present)
        """)

    st.markdown(f"### SUSY Regime Rankings at **{win_labels.get(selected_win, f'{selected_win}d')}** window")
